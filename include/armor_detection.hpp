...
#define POINT_DIST(p1,p2) std::sqrt((p1.x-p2.x)*(p1.x-p2.x)+(p1.y-p2.y)*(p1.y-p2.y))   //计算两点之间距离
...
struct Light : public cv::RotatedRect     //灯条结构体
{
    ...
    explicit Light(cv::RotatedRect &box) : cv::RotatedRect(box)   
    {
        ...
        angle = atan2(bottom.y-top.y, bottom.x-top.x) * 180 / CV_PI;   //atan2返回原点到两点的夹角   CV_PI是3.14
        // angle = box.angle;
...
struct Armor : public cv::RotatedRect    //装甲板结构体
{
    ...
    cv::Point2f armor_pt4[4]; //左下角开始逆时针    装甲板四个点
    double confidence;
    int id;  // 装甲板类别
    int grade;
    int type;  // 装甲板类型
...
private:
    ...
    bool isLight(Light& light, std::vector<cv::Point> &cnt);   //筛选灯条，选取符合条件的灯条
