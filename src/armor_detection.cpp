...
bool ArmorDetector::isLight(Light &light, vector<Point> &cnt)   //寻找能成为灯条的矩形，进行初步筛选
{
...
void ArmorDetector::findLights()   //筛选灯条，进行进一步精细的筛选
{
                ...
                0 <= rect.y && 0 <= rect.height && rect.y + rect.height <= _src.rows)
            {
                ...
                for (int i = 0; i < roi.rows; i++)
                {
                    for (int j = 0; j < roi.cols; j++)
                    {
                        ...
                        {
                            sum_r += roi.at<cv::Vec3b>(i, j)[2];     //用于bgr三通道的修改像素矩阵
...
void ArmorDetector::chooseTarget()   //打分机制选择最优打击的装甲板
{
    ...
    else if (candidateArmors.size() == 1)
    {
        ...
        detectNum(candidateArmors[0]);  //返回id
...
vector<Armor> ArmorDetector::autoAim(const cv::Mat &src)    //自瞄  具体里面用法要看
{
    if (enemy_color == RED)
        binThresh = 40;
    else
...
void ArmorDetector::detectNum(Armor &armor) // ??
{
    ...
    cv::warpPerspective(numSrc, numDst, rotation_matrix, cv::Size(warp_width, warp_height));   //图像的透视变换
...
bool ArmorDetector::conTain(RotatedRect &match_rect, vector<Light> &Lights, size_t &i, size_t &j)
{
    ...
    for (size_t k = i + 1; k < j; k++)
    {
        Point2f lightPs[4];
        // 灯条五等份位置的点
        if (matchRoi.contains(Lights[k].top) ||           //判断点是否在lights里面 为什么是或者
